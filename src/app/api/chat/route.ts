...
- Zero Industries（旧Bedrock Space）: 共同創業・COO（2026年2月〜）。CEOから直接COOを頼まれて参画。
...
function buildSystemPrompt(): string {
  ...
  if (posts.length === 0) return SYSTEM_PROMPT;

  const blogSection = posts
    .map((post) => {
      const body = post.content.slice(0, 1000);
      return `## ${post.title}（${post.date}）\nURL: https://koyatani.com/blog/${post.slug}\n${body}`;
    })
    .join("\n\n");

  return (
    SYSTEM_PROMPT +
    `\n---\n\n【ブログ記事】\n以下は谷昊埜が書いたブログ記事です。質問に関連する記事があれば内容を参考に回答し、記事のURLも紹介してください。\n\n${blogSection}\n`
  );
}
